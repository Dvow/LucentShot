...
// C FFI bindings use C naming conventions; dead_code removed per project policy
...
lazy_static! {
    ...
    static ref API: Api = {
        ...
        // 1. Write library files to temp (avoids polluting exe directory)
...
pub fn get_api() -> &'static Api {
    let api = &API;
    // Reference handles so compiler sees they're retained (keeps DLLs loaded)
